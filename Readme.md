...
* `[Key]` - this property represents a database-generated identity/key
    ...
    The grammar from the previous version has been set as Obsolete.
    ```csharp
    public class Employee
    {
        ~~[Key]~~
...
* ~~`[ExplicitKey]`~~ - this property represents an explicit identity/key which is 
  *not* automatically generated by the database. The grammar from the previous version has been set as Obsolete, please use Key(false).
...
* `[Write(WriteMode.Insert)]` -  this property is writeable ONLY for INSERT.
* `[Write(WriteMode.Update)]` -  this property is writeable ONLY for UPDATE.
* `[Write(WriteMode.Both)]` OR NOT SET -  this property is writeable BOTH for INSERT AND UPDATE.
